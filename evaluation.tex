...
% Feb6 update:  the sort is now as lazy as I can make it.  I still have to
% sort because of a place where C<M>=T is done with M and T both sparse,
% in the backtrack phase (W<S[i]>=bc_update./paths).  The frontier S[i] still
% needs sorting, and T = <S[i]>bc_update./paths constructs a jumbled T.
% I need to try to wait(S[i]) before this step; then I need only 1 sort.

...
%       \todo{do we need everything, e.g. subassign, Kronecker? Tim's comment: if we have space, I suggest we add all operations.
        Operations can be modified via a descriptor;
        matrices can be transposed ($\grbm{B}\grbt$),
        the mask can be complemented, and
        the mask can be valued (shown above) or structural ($\grbm{C}\grbmask{\grbstr{M}}$).
        A structural mask can also be complemented ($\grbm{C}\grbmask{\grbneg\grbstr{M}}$).

...
modify all internal components, unlike the way GraphBLAS objects behave.

The primary components are a GraphBLAS matrix named \verb'A' and an enumeration \verb'kind'.
The kind indicates how the matrix should be interpreted.
Currently, the only kinds defined are \verb'LAGRAPH_ADJACENCY_UNDIRECTED' and \verb'LAGRAPH_ADJACENCY_DIRECTED',
but more options will be added in the future.
...
algorithm call signatures. Utility functions exist to compute each cached property.
...
\begin{lstlisting}[language=C, label=lst:lagraphnew, caption=Constructing an LAGraph\_Graph object]
LAGraph_Graph G;
...
\begin{lstlisting}[language=C, label=lst:lagraphtrycatch, caption=LAGraph Try/Catch Utility Macro]
